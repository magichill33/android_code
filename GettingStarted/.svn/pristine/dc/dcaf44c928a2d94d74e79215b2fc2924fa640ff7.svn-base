...
public class LiloStreetActivity extends Activity implements ImageGetListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				LiloStreetActivity.this.finish();
			}
		});
		...
		viewHandler = new Handler(){
			...
			public void handleMessage(Message msg) {
	            ...
	            case MessageEnum.QUERY_FAILED:
	            	//progressDialog.dismiss();
	            	dismissProgress();
	                Toast.makeText(LiloStreetActivity.this,"该区域没有实景信息", Toast.LENGTH_LONG).show();
	                break;
	            default:
	                break;
	            }
		...
		if(reportType.equals("1"))
		{
			...
			try {
				...
				ldTV.addMarker("1001", "ly",
						...
						BitmapFactory.decodeResource(LiloStreetActivity.this.getResources(), R.drawable.light_red));
			} catch (TrueMapException e) {
				e.printStackTrace();
			}
		}
    ...
    public void showProgressDialog() {
        if (progressDialog == null) {
            progressDialog = ProgressDialog.show(LiloStreetActivity.this,"", getResources().getString(R.string.querying),
                    true);
...
/*    public void addOverlay(Point2D cp)
    {
    	OverlayItem item = new OverlayItem(cp, "", "");
		if (!mapView.getOverlays().contains(streetOverlay)) {
			mapView.getOverlays().add(streetOverlay);
			streetOverlay.addItem(item);
			mapView.invalidate();
		}
    }
    ...
    {
    	if(streetOverlay.size() > 0)
    	{
    		streetOverlay.clear();
    		mapView.getOverlays().remove(streetOverlay);
    	}
